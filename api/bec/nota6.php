...
if($_SERVER['REQUEST_METHOD']=='GET'){
    if(isset($_GET['id'])){
        $iduser=$_GET['id'];
        $query="SELECT * from horario6 where id_B='$iduser'";
        $resultado=metodoGet($query);
        echo json_encode($resultado->fetch(PDO::FETCH_ASSOC));
    }else{
        $query="SELECT * from horario6";
        $resultado=metodoGet($query);
...
if($_POST['METHOD']=='PUT'){
    ...
    ////
    $query="UPDATE horario6 SET nota_a1='$nota_a1',nota_b1='$nota_b1',nota_c1='$nota_c1',nota_d1='$nota_d1',
